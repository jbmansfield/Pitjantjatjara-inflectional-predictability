...
def main():
	...
	'''for mps in ["IMP.PFV", "PST.PFV", "PRS", "IMP.IPFV", "PST.IPFV", "FUT", "CHAR", "NMLZ", "MV"]:
		prop = mpsFreqs[mps] / sumTokens
		print(mps)
		print(str(prop))'''

	input = open('pitjantjatjara-verb-infl.txt', 'r')
	output = open('integrative-complexity_prosFirst.txt', 'w')
	...
	for sourceCell in cells:
		...
		for targetCell in targetCells:
			...
			for cue in ('cueless', 'pros', 'prosAug', 'prosAugSuff'):

				# a little hack to deal with the NG-class, where stems can be either monomoraic, or polymoraic/extrametrical. So they need to match against both N1-class and N2-class, which however don't match each other.
				if (sourceCell.vc=='ICNG' and cue=='pros'):
					sourceAnalogues = [cell for cell in cells if ( (cell.mps == sourceCell.mps) and ( (cell.vc=='ICN2') or (cell.vc=='ICNG')) )]
				elif (sourceCell.vc=='ICN1' and cue=='pros'):
					sourceAnalogues = [cell for cell in cells if ( (cell.mps == sourceCell.mps) and ( (cell.vc=='ICN1') or (cell.vc=='ICNG')) )]
				elif (sourceCell.vc=='ICN2' and cue=='pros'):
					sourceAnalogues = [cell for cell in cells if ( (cell.mps == sourceCell.mps) and ( (cell.vc=='ICN2') or (cell.vc=='ICNG')) )]
				else:
				...
				if len(sourceAnalogues)==1:
					...
					for analog in sourceAnalogues:
						...
						#output.write("Cand: "+analog.vc+"\t"+str(vcFreqs[analog.vc])+"\n")
